...
def test_subroutine_code():
    assert g.subroutine("test").code()[:2] == ['// subroutine test starts here', '(test)']

def test_gt_code():
    assert g.sub(g.var("i"), 100).code() == ['@100', 'D=A', '@i', 'D=M-D']

def test_assign_add_var():
    assert g.assign(g.var("sum"), g.add(g.var("sum"), g.var("i"))).code() == ['@i', 'D=M', '@sum', 'D=M+D', '@sum',  'M=D']

def test_while_comp():
    assert g.while_loop(g.ge(g.var("R6"))).code()[1:4] == []
...
def test_generator():
    ...
    f = g.for_list(g.var("R5"), [10000, 1000, 100, 10])
